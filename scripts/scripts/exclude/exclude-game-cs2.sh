...
#
#	----------------------------------------------------------------------------
#	Build a backup exclusion list for a Counter-Strike 2 (CS2) game-server
#	============================================================================
#	Created:       2024-03-07, by Weasel.SteamID.155@gMail.com        
#	Last modified: 2024-03-25, by Weasel.SteamID.155@gMail.com
#	----------------------------------------------------------------------------
#
#	Purpose:
#	
#		Build a backup exclusion list for Counter-Strike 2 (CS2) game server content.
#
...
#		Example:
#		
#			./exclude-game-cs2.sh gameserverid;
#	
...
GAME_APPID="730";															# what "AppID" (i.e. which game) to install.
MOD_SUB_FOLDER="game/csgo";													# which sub-folder of the installation folder the game shows-up inside of.
...
INSTALL_FOLDER="$TEMPWORK_FOLDER/stock-cs2";								# temp folder where game-server will be installed.
...
if [ -e "$CHECK_CONTROL_FILE" ]; then
		...
		cd $BASE_FOLDER/;
		#
        #	Normal version of TREE process, just normal Source-based games ...
        #tree -f --noreport -i -n | sed "s,\.\/,$GAME_SERVER\/$MOD_SUB_FOLDER\/," | grep "\." | sed '1d' | grep -Ev '.cfg|.txt|.ini' > $EXCLUDE_LIST_FILE;
		#
        #	Special version of TREE process, just for CS@ ...
		tree -f --noreport -i -n | sed "s,.\/,$GAME_SERVER\/game\/csgo\/," | grep "\." | sed '1d' | grep -Ev '.cfg|.txt|.ini' > $EXCLUDE_LIST_FILE;
